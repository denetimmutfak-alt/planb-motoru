...
def calculate_holding_period(df, signal_type: str) -> str:
    """🎯 TUTMA SÜRESİ ANALİZİ - Teknik göstergelere göre optimal süre"""
    try:
        ...
        if signal_type == "AL":
            ...
            return "1-3 gün" if current_rsi < 30 else "3-7 gün"
        else:
            return "1-2 hafta"
    except Exception:
        return "1-2 hafta"
...
def analyze_symbol_fast(symbol: str) -> dict | None:
    """Sembol için hızlı skor analizi + TUTMA SÜRESİ; başarısızsa None döner."""
    try:
        ...
        # TUTMA SÜRESİ hesapla
        signal_type = "AL" if "AL" in signal else ("SAT" if "SAT" in signal else "TUT")
        holding_period = calculate_holding_period(df, signal_type)
